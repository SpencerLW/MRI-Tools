function [ doneflag,m0file ] = pcasl_slw_v1( pcasldir )
...
dirout=dir([pcasldir,'/*SOURCE*pCASL*.PAR']);
aslfile=dirout(1).name;

dirout=dir([pcasldir,'/*pCASL*M0*.PAR']);
m0file=dirout(1).name;
...
%%% Get asl mask. M0 THRESHOLDING NOT BET
m0max=max(m0(:));
m0mask=m0>0.3*m0max;
%%%
...
tempnii=make_nii(flipud(rot90(cbf,3)),[3 3 8]);
save_nii(tempnii,[pcasldir,'/CBF.nii.gz']);

% save_avw_slw(rot90(cbf,3),[pcasldir,'/CBF.nii.gz'],'d',[3 3 7]);

dataFile=fopen([pcasldir,'/pCASL_readme.txt'],'w');
fwrite(dataFile,['slicetime_increment: ',num2str(slicetime_increment),newline,'partition coefficient: ', num2str(lambda), ...
